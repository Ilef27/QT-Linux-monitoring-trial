...
QString CPU::PrintStats(const std::vector<CPUData> & entries1, const std::vector<CPUData> & entries2)
{
    ...
    for(size_t i = 0; i < NUM_ENTRIES; ++i)
    {
      ...
      //  std::cout.setf(std::ios::fixed, std::ios::floatfield);
        ...
        ch= ch+ to_string((100.f * ACTIVE_TIME / TOTAL_TIME))+ "%";
       // std::cout << (100.f * ACTIVE_TIME / TOTAL_TIME) << "%";
        ch= ch+ " - idle: ";
        ch = ch + to_string(100.f * IDLE_TIME / TOTAL_TIME) + "%" +"\n";
